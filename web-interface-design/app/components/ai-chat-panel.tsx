...
export function AIChatPanel({ context = "general", stakeholders = ["Supervisor", "PM", "Artist"], onAction }: AIChatPanelProps) {
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      const aiResponse: Message = {
        ...
        suggestions: ["繼續調整 Lighting", "檢查 Composition 參考", "提交給 Supervisor 審核"],
        actions: [
  ...
  const getStakeholderPerspective = (stakeholder: string): string => {
    const perspectives: Record<string, string> = {
      "Supervisor": "從 Supervisor 的角度來看，這個鏡頭的情緒張力還不夠強。建議加強光影對比，讓觀眾更能感受到角色的內心掙扎。",
      "PM": "從專案管理的角度，這個版本已經符合基本規格要求。但考慮到時程，建議先確認 Supervisor 的核心意見後再進行修改。",
  ...
  const getStakeholderSuggestions = (stakeholder: string): string[] => {
    const suggestions: Record<string, string[]> = {
      "Supervisor": ["加強主光源對比", "調整角色視線方向", "增加環境氛圍"],
